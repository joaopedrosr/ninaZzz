...
CAMPOS = {"acordou": "acordou", "acordar": "acordou", "fim": "acordou",
          "dormiu": "adormeceu", "adormeceu": "adormeceu", "pegou": "adormeceu",
          "inicio": "inicio", "comecou": "inicio", "tentativa": "inicio"}


def a_editar(c, args):
    s = c.execute("SELECT * FROM sessoes WHERE status != 'cancelada' ORDER BY id DESC LIMIT 1").fetchone()
    if not s:
        return "Não tem nenhum sono pra editar ainda 🤷"

    def descreve(s):
        partes = [f"{'🛏️ Noite' if s['tipo'] == 'noite' else '💤 Soneca'}", f"começou {hm(dt(s['inicio']))}"]
        if s["adormeceu"]:
            partes.append(f"dormiu {hm(dt(s['adormeceu']))}")
        if s["acordou"]:
            partes.append(f"acordou {hm(dt(s['acordou']))}")
        return " · ".join(partes)

    if not args:
        return (f"✏️ Último sono: {descreve(s)}\n\nPra corrigir:\n"
                f"/editar acordou 07:50\n/editar dormiu 06:05\n/editar inicio 05:58\n/editar noite (ou soneca)")

    campo = norm(args[0])
    if campo in ("noite", "soneca") or (campo == "tipo" and len(args) > 1):
        novo = campo if campo in ("noite", "soneca") else norm(args[1])
        if novo not in ("noite", "soneca"):
            return "Tipo tem que ser noite ou soneca 😉"
        c.execute("UPDATE sessoes SET tipo=? WHERE id=?", (novo, s["id"]))
        return f"✏️ Pronto, virou {novo}. Reclassificado com sucesso 🗂️"

    col = CAMPOS.get(campo)
    ts, _, explicito = parse_hora(args[1:])
    if not col or not explicito:
        return "Não entendi 🤔 Manda assim: /editar acordou 07:50 (ou dormiu / inicio)"
    if col == "acordou" and not s["acordou"]:
        return f"Ela ainda tá registrada dormindo. Usa /acordou {hm(ts)} que dá no mesmo 😉"
    if col == "adormeceu" and not s["adormeceu"]:
        return f"Ela ainda nem dormiu nesse registro. Usa /dormiu {hm(ts)} 😉"

    novo = dict(s)
    novo[col] = ts.isoformat()
    ini, ad, ac = dt(novo["inicio"]), dt(novo["adormeceu"]), dt(novo["acordou"])
    if ad and ini > ad:
        if col == "adormeceu":
            novo["inicio"] = ts.isoformat()  # dormiu antes do "início"? então começou ali
            ini = ts
        else:
            return f"Ops, o início ({hm(ini)}) ficaria depois de ela dormir ({hm(ad)}) 🤨"
    if ad and ac and ac <= ad:
        return f"Ops, ela acordaria ({hm(ac)}) antes de dormir ({hm(ad)}). Viagem no tempo não vale ⏳"
    c.execute("UPDATE sessoes SET inicio=?, adormeceu=?, acordou=?, estimado=? WHERE id=?",
              (novo["inicio"], novo["adormeceu"], novo["acordou"],
               0 if col == "adormeceu" else s["estimado"], s["id"]))
    s = c.execute("SELECT * FROM sessoes WHERE id=?", (s["id"],)).fetchone()
    txt = f"✏️ Corrigido! {descreve(s)}"
    if s["adormeceu"]:
        txt += f"\n😴 Agora conta {dur(resumo(s, eventos(c, s['id']))['total'])} de sono. Precisão de relojoeiro suíço 🇨🇭"
    return txt
...
PALAVRAS = {
    ...
    "janela": "janela", "janelasono": "janela",
    "editar": "editar", "editarultimo": "editar", "corrigir": "editar", "corrige": "editar",
    "status": "status", "hoje": "hoje", "semana": "semana",
}
MUDAM_ESTADO = {"dormir", "noite", "soneca", "dormiu", "despertou", "voltou", "acordou", "editar",
                "cancelar", "desfazer"}
...
/janelasono — quanto tempo ela ainda aguenta acordada
/editar — corrige horários do último sono
/cancelar · /desfazer
/status · /hoje · /semana
...
async def post_init(app: Application):
    await app.bot.set_my_commands([
        ...
        BotCommand("editar", "Corrigir último sono"), BotCommand("nota", "Anotação"),
...
def main():
    ...
    for cmd in ["editar", "editarultimo", "dormir", "noite", "soneca", "dormiu", "despertou", "voltou", "acordou",
