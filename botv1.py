...
def a_dormir(c, ts, chat_id, args, jq, tipo=None):
    ...
    emoji = "🛏️" if tipo == "noite" else "💤"
    txt = f"{emoji} {tipo.capitalize()} começando às {hm(ts)}" + (f" ({local})" if local else "") + "."
...
def a_dormiu(c, ts, chat_id, args, jq):
    ...
    if not s:  # dormiu sem aviso (carro, colo...)
        ...
        return f"😴 Anotado: {NOME} apagou às {hm(ts)} ({tipo}" + (f", {local}" if local else "") + ")."
    if s["status"] == "despertou":
        return a_voltou(c, ts, chat_id, args, jq)
    if s["status"] == "dormindo":
        return "Ela já tá marcada como dormindo 😉"
    cancela_lembretes(jq, s["id"])
    ini = dt(s["inicio"])
    ts = max(ts, ini)
    ...
    return f"😴 {NOME} dormiu às {hm(ts)} — levou {dur(ts - ini)} pra pegar no sono."
...
def a_despertou(c, ts, chat_id, args, jq):
    ...
    return f"🌙 Despertou às {hm(ts)} — tinha dormido {dur(ts - inicio_bloco)} seguidos."
...
def a_voltou(c, ts, chat_id, args, jq):
    ...
    return f"😴 Voltou a dormir às {hm(ts)} — ficou {dur(ts - dt(ultimo['ts']))} acordada."
...
def a_acordou(c, ts, chat_id, args, jq, explicito=False):
    ...
    if r["mamadas"]:
        linhas.append(f"🍼 {r['mamadas']} mamada(s)")
...
async def lembrete(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not s or s["status"] != "tentando":
        return
    if d["i"] == 0:
        txt = f"⏱️ {d['min']} min — {NOME} deve estar começando a pegar no sono 🥱"
    elif d["i"] < len(LEMBRETES) - 1:
        txt = f"⏱️ {d['min']} min. Já dormiu?"
    else:
        txt = f"⏱️ {d['min']} min. Se ela apagou, marca aí 👇 Se ainda tá acordada, força aí 💪"
...
def grafico(dias, por, todos_blocos, mamadas):
    ...
    for i, d in enumerate(dias):
        ...
        mm = [(m - base).total_seconds() / 3600 for m in mamadas if base <= m < topo]
        if mm:
            ax.scatter(mm, [i] * len(mm), marker="v", color="crimson", zorder=3, s=35)
    ...
    ax.legend(handles=[Patch(color=COR_N, label="noite"), Patch(color=COR_S, label="soneca"),
                       Line2D([], [], marker="v", color="crimson", ls="", label="mamada")],
...
def texto_semana(dias, por, prev):
    ...
    linhas = [f"📊 Semana da {NOME} ({len(com)} dias com registro)",
...
MUDAM_ESTADO = {"dormir", "noite", "soneca", "dormiu", "despertou", "voltou", "acordou", "mamada",
                "cancelar", "desfazer"}
...
async def executar(update: Update, context: ContextTypes.DEFAULT_TYPE, acao, args):
    ...
    with db() as c:
        if acao in ("dormir", "noite", "soneca"):
            ...
            txt = a_mamada(c, ts, chat_id, resto, jq)
        elif acao == "nota":
...
async def post_init(app: Application):
    await app.bot.set_my_commands([
        ...
        BotCommand("mamada", "Registrar mamada"), BotCommand("nota", "Anotação"),
...
def main():
    ...
    for cmd in ["dormir", "noite", "soneca", "dormiu", "despertou", "voltou", "acordou", "mamada",
